...
The file store (e.g., https://zenodo.org/record/3630188)
is published anually for each SV-COMP. It stores
all verification tasks and witness files used and produced by any verifier
in that iteration of SV-COMP. To avoid content duplicates and naming conflicts,
the files are stored by their sha256 hash.

The results archive of the corresponding SV-COMP (e.g., https://zenodo.org/record/3630205)
...
This script takes these json files and recreate the original structure.
It uses hardlinks to do so.

Example usage:
```
contrib/unpackFileStores.py results-verified/*fileHashes.json.gz results-validated/*fileHashes.json.gz
```

See `contrib/unpackFileStores.py --help` for information about the command-line interface.
