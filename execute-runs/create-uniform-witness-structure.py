...
def unify_directories(target_file_glob, expected_name_in_files_dir, *dirs):
    for directory in dirs:
        ...
        logging.info(
            "  Processing files (unify_directories) in %s using pattern %s and new name %s ...",
            directory,
            target_file_glob,
        ...
        for i in glob.iglob(glob_pattern, recursive=True):
            ...
            assert ".yml" in abs_path, (
                "Found file is not within a *.yml dir: %s" % abs_path
            )
...
def main(argv=None):
    ...
    unify_directories(args.glob_pattern, args.new_name, *args.dirs)
